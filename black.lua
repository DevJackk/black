...
devlopertan = Redis:sismember(black.."black:devlopertan:Groups",UserId) 
...
elseif devlopertan then
Status = Redis:get(black.."black:devlopertan:Bot:Reply"..ChatId) or 'المطور الثانوي'
...
elseif Redis:sismember(black.."black:devlopertan:Groups",msg.sender.user_id) == true then
...
if msg.forward_info and not msg.devlopertanthen then -- التوجيه
local Fwd_Group = Redis:get(black.."black:Lock:forward"..msg_chat_id)
if Fwd_Group == "del" then
LuaTele.deleteMessages(msg.chat_id,{[1]= msg.id})
elseif Fwd_Group == "ked" then
...
Redis:srem(black.."black:devlopertan:Groups",UserId_Info.id) 
...
if not Redis:sismember(black.."black:devlopertan:Groups",Message_Reply.sender.user_id) then
...
Redis:srem(black.."black:devlopertan:Groups",Message_Reply.sender.user_id) 
...
if not Redis:sismember(black.."black:devlopertan:Groups",UserId) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId,"❍تم تنزيله مطور ثانوي مسبقا ").Reply,"md",true)  
else
Redis:srem(black.."black:devlopertan:Groups",UserId) 
...
if Redis:sismember(black.."black:devlopertan:Groups",UserId_Info.id) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId_Info.id,"❍تم ترقيته مطور ثانوي مسبقا ").Reply,"md",true)  
else
Redis:sadd(black.."black:devlopertan:Groups",UserId_Info.id) 
...
if Redis:sismember(black.."black:devlopertan:Groups",Message_Reply.sender.user_id) then
...
Redis:sadd(black.."black:devlopertan:Groups",Message_Reply.sender.user_id) 
...
if Redis:sismember(black.."black:devlopertan:Groups",UserId) then
return LuaTele.sendText(msg_chat_id,msg_id,Reply_Status(UserId,"❍تم ترقيته مطور ثانوي مسبقا ").Reply,"md",true)  
else
Redis:sadd(black.."black:devlopertan:Groups",UserId) 
...
local Info_Members = Redis:smembers(black.."black:devlopertan:Groups") 
...
local reply_markup = LuaTele.replyMarkup{
type = 'inline',
data = {{{text = '- مسح المطورين الثانويين', data = msg.sender.user_id..'/devlopertan'},},}}
...
Rink = 1
elseif Redis:sismember(black.."black:devlopertan:Groups",msg.sender.user_id)  then
Rink = 2
...
Rink = 1
elseif The_ControllerAll(Message_Reply.sender.user_id) == true then
Rink = 2
...
dev = "المطور ،" else dev = "" end
if Redis:sismember(black.."black:devlopertan:Groups",UserId_Info.id) then
dev = "النطور الثانوي ،" else devv = "" end
if Redis:sismember(black.."black:TheBasics:Group"..msg_chat_id, UserId_Info.id) then
crr = "منشئ اساسي ،" else crr = "" end
if Redis:sismember(black..'black:Originators:Group'..msg_chat_id, UserId_Info.id) then
cr = "منشئ ،" else cr = "" end
...
Redis:del(black.."black:devlopertan:Groups") 
...
elseif Text and Text:match('(%d+)/devlopertan') then
local UserId = Text:match('(%d+)/devlopertan')
if tonumber(IdUser) == tonumber(UserId) then
Redis:del(black.."black:devlopertan:Groups") 
